 ...
 * renders portfolio/holdings/prices, and places market orders. Prices are
 * lazily refreshed server-side on each load (TTL-cached), so this page
 * doubles as the "near real-time" surface — with honest stale/market-closed
 * labels instead of pretending the feed is live.
 */
(function () {
  "use strict";
  ...
  var state = { token: null, instruments: [], portfolio: null, marketOpen: false };
  ...
  async function loadAll() {
    ...
    var portfolio = null;
    try {
      portfolio = await api("/api/invest/portfolio/");
    } catch (err) {
      if (err.message === "unauthorized") throw err;
      /* portfolio 503 if prices unavailable — still show instruments */
    }
    state.instruments = instrumentsRes.instruments || [];
    state.marketOpen = !!instrumentsRes.market_open;
  ...
  function renderMarketStatus() {
    ...
    dot.className = "market-dot" + (state.marketOpen ? " market-dot--open" : " market-dot--closed");
    label.textContent = state.marketOpen
      ? "NSE market is open (9:15–15:30 IST). Prices may be delayed by a few minutes."
      : "NSE market is closed. Showing last known prices — trades fill at the last close.";
  }
  ...
  function updateOrderHint() {
    ...
    var verb = side === "buy" ? "will cost" : "will credit";
    el.textContent = side === "buy"
      ? "This order " + verb + " approximately " + money(total) + " at the current price."
      : "This order " + verb + " approximately " + money(total) + " at the current price.";
  }
        ...
        p.holdings.map(function (h) {
          ...
          return (
            "<tr>" +
              ...
              '<td class="num">' + money(h.last_price) + (h.stale ? ' <span class="stale" title="delayed/stale quote">≈</span>' : "") + "</td>" +
  ...
  function renderInstruments() {
    ...
    wrap.innerHTML =
      '<table class="inv-table">' +
        ...
        "<th>As of (IST)</th><th>Status</th></tr></thead><tbody>" +
        state.instruments.map(function (i) {
          ...
          var status = i.stale
            ...
            : (i.as_of ? '<span class="ok">live-ish</span>' : '<span class="stale">default</span>');
          return (
            "<tr>" +
  ...
  async function resetPortfolio() {
    ...
    if (!window.confirm("Reset your portfolio? This wipes all holdings and orders and restores your starting balance.")) return;
    try {
  ...
  function bind() {
    ...
    document.getElementById("order-instrument").addEventListener("change", updateOrderHint);
